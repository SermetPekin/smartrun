...
class TestLocalRequests:
    ...
    @pytest.mark.skipif(not_long_test(), reason="Long test - set SMARTRUN_LONGTEST=1 to enable")
    def test_local_requests_google(self):
        ...
        # Basic assertions
        assert response["success"], f"Request failed: {response.get('error', 'Unknown error')}"
        ...
        assert isinstance(response["response_time"], (int, float)), "Response time should be numeric"
        
        print(f"✅ SUCCESS - Status: {response['status']}, Time: {response['response_time']}s")
    ...
    def test_check_url_status(self):
        ...
        if status["accessible"]:
            assert status["status"] == 200, f"Expected status 200, got {status['status']}"
            assert isinstance(status["response_time"], (int, float)), "Response time should be numeric"
            print(f"✅ URL Status Check - Accessible: {status['accessible']}, Status: {status['status']}")
        else:
    ...
    def test_invalid_url_handling(self):
        ...
        for url in invalid_urls:
            ...
            assert "error" in response, f"Failed request should include error for URL: {url}"
    ...
    def test_nonexistent_domain(self):
        ...
        assert "URL Error" in response["error"] or "HTTP Error" in response["error"], \
    ...
    def test_http_error_codes(self):
        ...
        for url, expected_status in test_cases:
            ...
            assert not response["success"], f"HTTP error {expected_status} should result in success=False"
    ...
    def test_response_structure(self):
        ...
        if response["success"]:
            ...
            pytest.skip(f"Skipping structure test due to connection failure: {response.get('error')}")
...
if __name__ == "__main__":
    ...
    try:
        ...
        if not_long_test():
            ...
            print("⚠️  Long tests disabled. Set SMARTRUN_LONGTEST=1 to enable network tests.")
